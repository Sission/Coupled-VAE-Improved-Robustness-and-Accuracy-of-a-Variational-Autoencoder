...
Referenced from https://github.com/hwalsuklee/tensorflow-mnist-VAE
"""
...
def main(args):
    ...
    # network architecture
    y, z, loss, neg_marginal_likelihood, KL_divergence, mu, sigma = vae.autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob)
    # print(mu.shape)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(total_batch):
                ...
                sigma1 = sess.run(sigma, feed_dict={x_hat: batch_xs_input, x: batch_xs_target, keep_prob : 0.9})
                mu1 = sess.run(mu, feed_dict={x_hat: batch_xs_input, x: batch_xs_target, keep_prob: 0.9})

            # print cost every epoch
            # print("epoch %d: L_tot %03.2f L_likelihood %03.2f L_divergence %03.2f" % (epoch, tot_loss, loss_likelihood, loss_divergence))
            print("epoch %d: L_tot %03.2f L_likelihood %03.2f" % (epoch, tot_loss, loss_likelihood))
    ...
    # np.savetxt('marginal_divergence', loss_divergence)
    np.savetxt('marginal_likelihood', loss_divergence)
    np.savetxt('sigma', sigma1)
    np.savetxt('mu', mu1)
